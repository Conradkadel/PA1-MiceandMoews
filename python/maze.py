...
class Maze(problem.Problem):
    # Keep this updated with all available heuristics.
    ...
    def loadFile(self, mazeFilename):
        ...
        Parameters:
            mazeFilename (str): The path to a file containing one character per 
                                spot on the board.
        '''
        self.board = []
    ...
    def drawableBoard(self):
        '''
        Returns (list((str, Any, float, float))): The maze as a board of RBG 
            colors rather than maze-specific symbols.
        '''
        colorMap = {
            ' ': search_driver.WHITE,
        ...
        for row in self.board:
            boardSpots.append([])
            for col in row:
                boardSpots[-1].append(BoardSpot(col, colorMap[col]))
    ...
    def getDistance(self, state):
        '''
        Estimates the distance from the state to the exit using whatever the 
        current heuristic is.

        Parameters:
            state ((int,int)): The state to check.
            
        Returns (float): The estimated distance from the given state to a goal 
            state. This should use whatever heuristic is currently set for the 
            problem.
        '''
