...
class BFS:
    ...
    def __init__(self, problem, graphSearch=False):
        ...
        Parameters:
            ...
            useGraphSearch (bool): If True, the search will not expand states that have already been seen.
        '''
        self.name = "BFS"
    ...
    def getMaxFringeSize(self):
        '''Returns (int): The max fringe size during the search.
        '''
    ...
    def expandNode(self, node):
        ...
        for move,successor,cost,dist in self.problem.successors(node.state):
            ...
            heapq.heappush(self.fringe, SearchNode(
                ...
                node.pathActions + [move],
                node.pathStates + [successor], 
                node.pathCost + cost, # Backwards cost g(x)
                node.pathCost + cost # Priority: g(x)
            ))
    ...
    def expandNode(self, node):
        ...
        for move,successor,cost,dist in self.problem.successors(node.state):
            ...
            heapq.heappush(self.fringe, SearchNode(
                ...
                node.pathStates + [successor], 
                node.pathCost + cost, ## Backward cost: g(x)
                node.pathCost + cost + dist ## f(x) = g(x) + h(x)
            ))
