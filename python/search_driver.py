...
class SearchDriver:
    ...
    def __init__(self, problem, searchStrategy, fps=FPS, spotSize=SPOT_SIZE, 
        ...
        Parameters:
        	...
        	searchStrategy (Any): A search strategy object (e.g, of type BFS, DFS, etc.)
    ...
    def loadPathImages(self):
        '''
        Loads the images used to display the solution path directions.
        '''
        downUpPath = pygame.image.load("images/down-up.png")
        leftRightPath = pygame.image.load("images/left-right.png")
        downRightLeftUpPath = pygame.image.load("images/downright-leftup.png")
        rightDownUpLeftPath = pygame.image.load("images/rightdown-upleft.png")
        leftDownUpRight = pygame.image.load("images/leftdown-upright.png")
        downLeftRightUpPath = pygame.image.load("images/downleft-rightup.png")
        ...
        self.pathImages = {
            ...
            'l': leftRightPath,
            'r': leftRightPath,
            'dr': downRightLeftUpPath,
            'lu': downRightLeftUpPath,
            'rd': rightDownUpLeftPath,
            'ul': rightDownUpLeftPath,
            'ld': leftDownUpRight,
            'ur': leftDownUpRight,
            'dl': downLeftRightUpPath,
            'ru': downLeftRightUpPath
        }
    ...
    def drawSpot(self, x, y, spot):
        ...
        if len(spot.color) == 4:
            s.set_alpha(spot.color[3])    # alpha level
        s.fill(spot.color[0:3])           # this fills the entire surface
        
        # Display any images associated with the spot.
        if spot.images is not None:
            for image in spot.images:
                img = pygame.transform.scale(image, (self.spotSize, self.spotSize))
                s.blit(img, (0,0))
        self.screen.blit(s, (externalX,externalY))


    def setSolutionPathGraphics(self, pathStates):
        '''
        Sets the graphics for the solution path to orange and overlays an image
        showing the direction of the path.

        Parameters:
            pathStates (list((int,int))): The list of states in the solution path.
        '''
        for i in range(1, len(pathStates)-1):
            prevSpot = pathStates[i-1]
            curSpot = pathStates[i]
            nextSpot = pathStates[i+1]

            if prevSpot[X] == nextSpot[X]:
                # Down
                if prevSpot[Y] < nextSpot[Y]:
                    actionPath = "d"
                # Up
                else:
                    actionPath = "u"
            elif prevSpot[Y] == nextSpot[Y]:
                # Right
                if prevSpot[X] < nextSpot[X]:
                    actionPath = "r"
                # Left
            ...
            # Down right
            elif nextSpot[X] > curSpot[X] and prevSpot[Y] < curSpot[Y]:
                actionPath = "dr"
            
            # Left up
            elif prevSpot[X] > curSpot[X] and nextSpot[Y] < curSpot[Y]:
                actionPath = "lu"
    ...
    def run(self):
        ...
        while not done:
            ...
            if not searchOver:
                node = self.searchStrategy.nextState()
                ...
                if not node:
                    ...
                    if self.graphicsOn:
                        ...
                        #         self.board[spot[0]][spot[1]].color = ORANGE
                        self.setSolutionPathGraphics(node.pathStates)
                    searchOver = True
                    

                    # Print out stats about the solution.
                    print(f'Search strategy: {self.searchStrategy.getName()}')
                    print(f'States expanded: {self.searchStrategy.getStatesExpanded()}')
                    print(f'Max fringe size: {self.searchStrategy.getMaxFringeSize()}')
                    ...
                    print(f'Solution path (actions): {"".join([a[0] for a in node.pathActions])}')
                        ...
                        self.board[node.state[0]][node.state[1]].color = curSpotColor.copy()
            ...
            # Ensures that we keep the window open if graphics is on, but exit
            # immediately if graphics is off.
            elif not self.graphicsOn:
                done = True
...
def main():
    '''
    Parses command line arguments and runs the search strategy. Call this file without any arguments
    ...
    searchProblems = {
        'maze': maze.Maze,
        'micemeow' : mice_and_meows.MiceAndMeows
    }
    ...
    for arg in sys.argv[1:]:
        ...
        ## Search strategy
            ...
            searchStrategy = arg[3:]
            if searchStrategy not in searchStrategies:
                sys.stderr.write('Invalid search strategy: '+searchStrategy+'\n')
                sys.stderr.write(USAGE)
                sys.exit()
